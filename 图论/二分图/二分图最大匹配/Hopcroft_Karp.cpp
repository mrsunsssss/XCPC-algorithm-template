namespace Hopcroft_Karp {
    ...
    struct Graph {
        ...
        vector<size_type> m_left_match, m_right_match, m_visit, m_dist;
        mutable vector<size_type> m_starts, m_edges;
        ...
        bool _dfs(size_type a) {
            ...
            for (size_type* cur = m_edges.data() + m_starts[a], *end = m_edges.data() + m_starts[a + 1]; cur != end; cur++) {
                size_type b = *cur;
                if (!~m_right_match[b]) {
                    m_right_match[b] = a, m_left_match[a] = b;
                    return true;
                }
            }
            for (size_type* cur = m_edges.data() + m_starts[a], *end = m_edges.data() + m_starts[a + 1]; cur != end; cur++) {
                size_type b = *cur;
                if (m_visit[m_right_match[b]] != m_time && m_dist[m_right_match[b]] == m_dist[a] + 1 && _dfs(m_right_match[b])) {
                    m_right_match[b] = a, m_left_match[a] = b;
                    return true;
                }
            }
        ...
        size_type calc() {
            _prepare();
            vector<uint32_t> queue(m_vertex_cnt);
            size_type res = 0;
            auto bfs = [&] {
                fill_n(m_dist.data(), m_vertex_cnt, -1);
        ...
        size_type find_right(size_type left) const {
            return m_left_match[left];
        }
        size_type find_left(size_type right) const {
            return m_right_match[right];
        }
    };
}

//how to use:
Hopcroft_Karp::Graph g(vertex, edge);
g.add_edge(x,y)
g.calc()
g.find_left(),g.find_right()
