...
// using namespace Segments;using namespace Lines;
// using namespace Circles;
// using namespace Convex_Hull;using namespace Polygon;
namespace Segments {
...
void Solve() {
    ...
    n = a.size();
    ld res = 4e18;
    int ans_i, ans_j, ans_l, ans_r;
    for (int i = 0, j = 1, r = 1, l;i < n;i++) {
        auto Line = uv(a[i], a[next(i, n)]);
        while (sgn(
            cross(Line, uv(a[i], a[next(j, n)])) -
            cross(Line, uv(a[i], a[j]))
        ) >= 0) {
            j = next(j, n);
        }
        while (sgn(
            dot(Line, uv(a[i], a[next(r, n)])) -
            dot(Line, uv(a[i], a[r]))
        ) >= 0) {
            r = next(r, n);
        }
        if (i == 0) l = r;
        while (sgn(
            dot(Line, uv(a[i], a[next(l, n)])) -
            dot(Line, uv(a[i], a[l]))
        ) <= 0) {
            l = next(l, n);
        }
        ld H = abs(cross(Line, uv(a[i], a[j])));
        ld B1 = abs(dot(uv(a[i], a[l]), Line));
        ld B2 = abs(dot(uv(a[next(i, n)], a[r]), Line));
    ...
    auto down_line = LD{ a[ans_i], a[next(ans_i, n)] };
    auto ty1 = project(down_line, a[ans_r]);
    auto ty2 = project(down_line, a[ans_l]);
    auto up_line = LD{ a[ans_j],add(a[ans_j],uv(a[ans_i],a[next(ans_i,n)])) };
    auto ty3 = project(up_line, a[ans_r]);
    auto ty4 = project(up_line, a[ans_l]);
    vector<PD> res_dot(4);
    res_dot[0] = ty1;
    res_dot[1] = ty3;
    res_dot[2] = ty4;
