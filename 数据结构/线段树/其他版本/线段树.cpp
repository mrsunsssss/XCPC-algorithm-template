...
struct Info {
    i64 sum = 0, one = 0, zero = 0, cone = 0, czero = 0, l = 0;
 
    void apply(Tag t, int len) {
        if (t.t1) {
            one = sum / 2;
            zero = sum - sum / 2;
            czero = 0, cone = sum, l = 1;
        }
        if (t.t2) {
            std::swap(one, zero);
            std::swap(cone, czero);
...
Info operator+(Info a, Info b) {
    Info c;
    c.sum = a.sum + b.sum;
    c.one = a.one + (a.sum & 1 ? b.zero : b.one);
    c.zero = a.zero + (a.sum & 1 ? b.one : b.zero);
    c.cone = a.cone + b.cone;
    c.czero = a.czero + b.czero;
    c.l = a.l;
    return c;
}
