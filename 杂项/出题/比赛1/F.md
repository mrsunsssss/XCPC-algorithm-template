...
游戏还有n秒钟进入决赛圈，山鸡王在进入决赛圈前的每秒必须选择两种操作之一：
...
请问山鸡王在保住性命的情况下，在决赛圈前最多可以为自己获得多少充能点数，以便他能够在决赛圈更轻松的获得胜利？
...
solution：
考虑贪心，当前仅当下一步如果使用操作1会使警惕值大于 $lim$ 时，才会使用操作2 。我们考虑找到令 $tx \mod y + x > lim$ 的最小整数 $t$ ，此时需要减去不足 $y$ 的警惕值，警惕值清零。这样就构成了一个循环。对于剩下的小块，由于总能减去完整的 $y$ ，可以看作是先一直进行操作1，再一直进行操作2，需要最后警惕值小于等于lim，这个操作直接使用二分即可。
